<?php
/**
 * Responsive Blocks Mailchimp API Class
 *
 * @package ResponsiveBlocks\Newsletter\Mailchimp
 */
 ...
 * Class Mailchimp
 *
 * @package ResponsiveBlocks\Newsletter
 */
final class Mailchimp implements Provider_Interface {
	...
	public function __construct( $api_key ) {
		...
		if ( empty( $api_key ) ) {
			throw new Mailchimp_API_Error_Exception(
				/* translators: %s This PHP class name. Will print ResponsiveBlocks\Newsletter\Mailchimp */
				sprintf( esc_html__( 'An API key is required to use %s.', 'responsive-blocks' ), __CLASS__ )
			);
		}
	...
	public function subscribe( $email, array $args = array() ) {
		...
		if ( empty( $email ) || ! is_email( $email ) ) {
			throw new Mailchimp_API_Error_Exception(
				esc_html__( 'An invalid email address was provided.', 'responsive-blocks' )
			);
		}
		...
		if ( empty( $list_id ) || ! in_array( $list_id, $list_ids, true ) ) {
			throw new Mailchimp_API_Error_Exception(
				/* translators: %s The PHP method name. Will be ResponsiveBlocks\Newsletter\Mailchimp\subscribe. */
				sprintf( esc_html__( 'An invalid Mailchimp list ID was provided in %s', 'responsive-blocks' ), __METHOD__ )
			);
		}
		...
		if ( ! $response || ( ! empty( $response['status'] ) && is_numeric( $response['status'] && 200 !== $response['status'] ) ) || $this->mailchimp->getLastError() ) {
			throw new Mailchimp_API_Error_Exception(
				...
				esc_html( sprintf( __( 'There was an error subscribing your email address. Please try again. Error: %s', 'responsive-blocks' ), $this->mailchimp->getLastError() ) ),
				$response['status']
			);
		}
	...
	private function cached_lists() {
		...
		$lists = get_transient( 'responsive_blocks_mailchimp_lists' );
		...
		if ( empty( $lists ) ) {
			...
			set_transient( 'responsive_blocks_mailchimp_lists', $lists, MINUTE_IN_SECONDS * 15 );
		}
