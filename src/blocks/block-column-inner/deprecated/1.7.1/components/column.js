...
export default class Column_1_7_1 extends Component {
	...
	render() {
		...
		if (attributes.customBackgroundColor) {
			backgroundColorClass = 'ra-has-custom-background-color';
		} else {
		...
		if (attributes.customTextColor) {
			textColorClass = 'ra-has-custom-text-color';
		} else {
		...
		return (
			<div
				className={ classnames(
					'ra-block-layout-column',
					attributes.columnVerticalAlignment ? 'ra-is-vertically-aligned-' + attributes.columnVerticalAlignment : null
				) }
			>
				<div
					className={ classnames(
						'ra-block-layout-column-inner',
