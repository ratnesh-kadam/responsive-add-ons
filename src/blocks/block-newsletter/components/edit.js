...
class Edit extends Component {
	...
	render() {
		...
		const apiKeyDefined = responsive_blocks_newsletter_block_vars.mailingListProviders.mailchimp.api_key_defined;

		/* Setup button background color class */
		let buttonBackgroundColorClass;

		if (attributes.customButtonBackgroundColor) {
			buttonBackgroundColorClass = 'ra-has-custom-background-color';
		} else {
		...
		if (attributes.customButtonTextColor) {
			buttonTextColorClass = 'ra-has-custom-text-color';
		} else {
		...
		return [
			...
			<NewsletterContainer { ...this.props }>
				{ ! apiKeyDefined && (
					<Fragment>
						<div className="ra-newsletter-notice">
							{ __( 'You must define your newsletter provider API keys to use this block.', 'responsive-blocks' ) }
							<p><a href={ responsive_blocks_newsletter_block_vars.plugin_settings_page_url }>{ __( 'Configure your settings', 'responsive-blocks' ) }</a></p>
						</div>
					</Fragment>
				) }
				{ apiKeyDefined && (
					<Fragment>
						<RichText
							tagName="span"
							className="ra-block-newsletter-label"
						...
						<TextControl
							name="ra-newsletter-email-address"
						/>
						...
						>
							...
							{ isSelected && (
								<form
									key="form-link"
									className={ `blocks-button__inline-link ra-button-${attributes.buttonAlignment}`}
