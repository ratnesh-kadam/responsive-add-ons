...
const ALLOWED_BLOCKS = [ 'responsive-blocks/ra-column' ];

/* Get the column template. */
const getLayoutTemplate = memoize( ( columns ) => {
	return _times( columns, () => [ 'responsive-blocks/ra-column' ] );
} );
...
class Edit extends Component {
	...
	render() {
		...
		const columnOptions = [
			{
				name: __( '1 Column', 'responsive-blocks' ),
				key: 'one-column',
				columns: 1,
				icon: icons.oneEqual,
			},
			{
				name: __( '2 Columns', 'responsive-blocks' ),
				key: 'two-column',
				columns: 2,
				icon: icons.twoEqual
			},
			{
				name: __( '3 Columns', 'responsive-blocks' ),
				key: 'three-column',
				columns: 3,
				icon:icons.threeEqual
			},
			{
				name: __( '4 Columns', 'responsive-blocks' ),
				key: 'four-column',
				columns: 4,
				icon: icons.fourEqual
			},
			{
				name: __( '5 Columns', 'responsive-blocks' ),
				key: 'five-column',
				columns: 5,
				icon: icons.fiveEqual,
			},
			{
				name: __( '6 Columns', 'responsive-blocks' ),
				key: 'six-column',
				columns: 6,
				icon: icons.sixEqual,
			},
		...
		if ( ! attributes.layout && this.state.selectLayout ) {
			return [
				<Placeholder
					key="placeholder"
					icon="editor-table"
					label={ attributes.columns ? __( 'Column Layout', 'responsive-blocks' ) : __( 'Column Number', 'responsive-blocks' ) }
					instructions={ attributes.columns ? sprintf( __( 'Select a layout for this column.', 'responsive-blocks' ) ) : __( 'Select the number of columns for this layout.', 'responsive-blocks' ) }
					className={ 'ra-column-selector-placeholder' }
				>
					{ ! attributes.columns ?
						<ButtonGroup
							aria-label={ __( 'Select Row Columns', 'responsive-blocks' ) }
							className="ra-column-selector-group"
						>
							{ map( columnOptions, ( { name, key, icon, columns } ) => (
								<Tooltip text={ name } key={ key }>
									<div className="ra-column-selector">
										<Button
					...
					:
						<Fragment>
							<ButtonGroup
								aria-label={ __( 'Select Column Layout', 'responsive-blocks' ) }
								className="ra-column-selector-group"
							>
								...
								<Button
									className="ra-column-selector-button-back"
									onClick={ () => {
										setAttributes( {
											columns: null,
										} );
								...
								>
									{ __( 'Return to Column Selection', 'responsive-blocks' ) }
								</Button>
							</ButtonGroup>
						</Fragment>
					}
