...
class LatestPostsBlock extends Component {
	...
	render() {
		...
		if ( ! hasPosts ) {
			return (
				<Fragment>
					...
					<Placeholder
						icon="admin-post"
						label={ __( 'Responsive Blocks Post and Page Grid', 'responsive-blocks' ) }
					>
						{ ! Array.isArray( latestPosts ) ?
							<Spinner /> :
							__( 'No posts found.', 'responsive-blocks' )
						}
					</Placeholder>
				</Fragment>
			);
		...
		const layoutControls = [
			{
				icon: 'grid-view',
				title: __( 'Grid View', 'responsive-blocks' ),
				onClick: () => setAttributes( { postLayout: 'grid' } ),
				isActive: attributes.postLayout === 'grid',
			},
			{
				icon: 'list-view',
				title: __( 'List View', 'responsive-blocks' ),
			...
			<Fragment>
				...
				>
					{ attributes.displaySectionTitle && attributes.sectionTitle &&
						<SectionTitleTag className="ra-post-grid-section-title">{ attributes.sectionTitle }</SectionTitleTag>
					}
					...
					<div
						className={ classnames( {
							...
							'ra-post-grid-items' : 'ra-post-grid-items'
						} ) }
					>
							...
							>
								{
									attributes.displayPostImage && post.featured_media ? (
										<PostGridImage
											{ ...this.props }
											imgAlt={ decodeEntities( post.title.rendered.trim() ) || __( '(Untitled)', 'responsive-blocks' ) }
								...
								<div className="ra-block-post-grid-text">
									<header className="ra-block-post-grid-header">
										{ attributes.displayPostTitle &&
											<PostTag className="ra-block-post-grid-title"><a href={ post.link } target="_blank" rel="bookmark">{ decodeEntities( post.title.rendered.trim() ) || __( '(Untitled)', 'responsive-blocks' ) }</a></PostTag>
										}

										{ isPost &&
											<div className="ra-block-post-grid-byline">
												{ attributes.displayPostAuthor && post.author_info.display_name &&
													<div className="ra-block-post-grid-author"><a className="ra-text-link" target="_blank" href={ post.author_info.author_link }>{ post.author_info.display_name }</a></div>
												}

												{ attributes.displayPostDate && post.date_gmt &&
													<time dateTime={ moment( post.date_gmt ).utc().format() } className={ 'ra-block-post-grid-date' }>
														{ moment( post.date_gmt ).local().format( 'MMMM DD, Y', 'responsive-blocks' ) }
													</time>
												}
											</div>
										}
									...
									<div className="ra-block-post-grid-excerpt">
										...
										{ attributes.displayPostLink &&
											<p><a className="ra-block-post-grid-more-link ra-text-link" href={ post.link } target="_blank" rel="bookmark">{ attributes.readMoreText }</a></p>
										}
									</div>
								</div>
							</article>
...
export default compose( [
	withSelect( ( select, props ) => {
		...
		const { getEntityRecords } = select( 'core', 'responsive-blocks' );
